...
function initPeriodSelect(type = 'monthly') {
    ...
    if (type === 'monthly') {
        ...
        (salesData.weekly || []).map(w => w.week).reverse().forEach(week => {
            select.innerHTML += `<option value="${week}">${week}</option>`;
        });
    } else if (type === 'daily') {
        (salesData.daily || []).map(d => d.date).reverse().slice(0, 60).forEach(date => {
