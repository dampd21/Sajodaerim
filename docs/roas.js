...
const UPDATED_AT_KEY = 'roas_dashboard_updated_at';
...
function renderUpdateTime() {
    ...
    const rows = loadFromStorage().length;
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: (c) => {
                            if (c.dataset.label.includes('ROAS')) return 'ROAS: ' + c.parsed.y.toFixed(0) + '%';
                            if (c.dataset.label.includes('전환')) return '전환: ' + c.parsed.y.toFixed(0) + '건';
...
function displayBudgetPie(m) {
    ...
    budgetPieChart = new Chart(ctx, {
        type: 'pie',
        data: { labels, datasets: [{ data, backgroundColor: ['#00d4ff', '#4ecdc4', '#ffe66d', '#ff6b6b', '#b794f6', '#666'], borderWidth: 0 }] },
        options: {
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
...
function displayFunnel(m) {
    ...
    const imp = m.total_impressions || 0;
    const clk = m.total_clicks || 0;
    const conv = m.total_conversions || 0;
    ...
    funnelChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { ticks: { color: '#a0a0a0', callback: v => formatCompact(v) }, grid: { color: 'rgba(255,255,255,0.05)' } },
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: (c) => {
                            if (c.dataset.label.includes('ROAS')) return 'ROAS: ' + c.parsed.y.toFixed(0) + '%';
                            if (c.dataset.label.includes('만')) return c.dataset.label + ': ' + c.parsed.y.toFixed(0) + '만원';
...
function displayTopTables(m) {
    ...
    document.getElementById('roasTopTable').innerHTML = [...cp]
        .sort((a, b) => b.roas - a.roas)
        .slice(0, 10)
        .map((c, i) =>
            '<tr class="campaign-row" onclick="showCampaignDetail(' + cp.indexOf(c) + ')">' +
                '<td>' + (i + 1) + '</td>' +
    ...
    document.getElementById('cvrTopTable').innerHTML = [...cp]
    ...
    document.getElementById('allCampaignsTable').innerHTML = cp.map((c, idx) => {
        ...
        return '<tr class="campaign-row" onclick="showCampaignDetail(' + idx + ')">' +
            ...
            '<td>' + c.cvr.toFixed(1) + '%</td>' +
            '<td>' + c.ctr.toFixed(1) + '%</td>' +
            '<td>' + c.cpa.toLocaleString() + '원</td>' +
            '<td>' + c.conversions.toLocaleString() + '건</td>' +
            '<td>' + formatCompact(c.spend) + '</td>' +
            '<td>' + formatCompact(c.revenue) + '</td>' +
            '<td><span class="status-badge status-' + c.status + '">' + sl + '</span></td>' +
        '</tr>';
    }).join('');
}
...
function showCampaignDetail(index) {
    ...
    const mh = c.ad_type === 'lead'
        ? '<div class="summary-cards roas-summary">' +
            ...
            '<div class="card"><div class="card-label">전환율</div><div class="card-value">' + c.cvr.toFixed(1) + '%</div></div>' +
          '</div>'
        : '<div class="summary-cards roas-summary">' +
            ...
            '<div class="card"><div class="card-label">전환수</div><div class="card-value">' + c.conversions.toLocaleString() + '건</div></div>' +
          '</div>';

    document.getElementById('campaignDetailContent').innerHTML =
        '<h2>' + c.campaign_name + '</h2>' +
        mh +
        '<div class="insight-box ' + ic + '" style="margin-top:20px;">' +
            '<strong>권장사항</strong><br><br>' + getRecommendation(c) +
        '</div>';
...
function normalizeData(jsonData) {
    ...
    return jsonData.map(function (row) {
        ...
        if (mapping.ad_type) {
            ...
            if (v.includes('lead') || v.includes('잠재') || v.includes('리드')) at = 'lead';
        }
...
function addManualData() {
    ...
    ['manualSpend', 'manualImpressions', 'manualClicks', 'manualConversions', 'manualRevenue'].forEach(function (id) {
    ...
    document.getElementById('manualDataPreview').innerHTML =
        '<div style="margin-top:10px;padding:10px;background:rgba(255,255,255,0.03);border-radius:6px;font-size:0.85rem;color:#aaa;">' +
            '<strong>마지막 입력:</strong> ' + last.date + ' | ' + last.campaign_name + ' | 지출: ' + last.spend.toLocaleString() + '원 | 클릭: ' + last.clicks +
        '</div>';
...
function applyDateFilter(ft) {
    ...
    if (ft === 'all') {
        ...
        var ma = new Date(today);
        ma.setDate(today.getDate() - 29);
...
function saveMemo() {
    ...
    if (text) {
        localStorage.setItem('roas_memo_' + currentMemoTarget, text);
        alert('메모 저장!');
    } else {
        localStorage.removeItem('roas_memo_' + currentMemoTarget);
        alert('메모 삭제!');
    }
...
function exportToExcel() {
    ...
    if (currentCampaignData.length > 0) {
        var ws2 = XLSX.utils.json_to_sheet(
            currentCampaignData.map(function (c) {
                return {
